...
public class Climb {
    ...
    public static double down1 = 195, down2 = 245, raise1 = 305, raise2 = 135;
    ...
    public static double BAR1 = 550, BAR2 = 910;
...
//                last_update = System.currentTimeMillis();
            ...
            case PULL_UP:
                ...
                if((Elevator.getCurrentPosition() < 8 || Math.abs(Elevator.motor.getVelocity()) < 1) && TimeSinceLastStateChange.seconds() > 0.5 + 0.2 + 1){
                    Elevator.setTargetPosition(0);
                    PutDown();
//                    disengagePTO();
...
//                if(TimeSinceLastStateChange.seconds() < 0.1 + 0.1) break;
            ...
            case PULL_ELEVATOR_A_BIT:
                ...
                engagePTO();
                if(!climbOv) {
                    if (Initialization.imu.getRobotYawPitchRollAngles().getPitch(AngleUnit.DEGREES) > -84 && !Enabled) {
                        Elevator.Disable = true;
                    }
                    else {
...
//                if(Elevator.getCurrentPosition() < 0) Elevator.setTargetPosition(0);
    ...
    public static boolean climbOv = false, Enabled = true;
}
