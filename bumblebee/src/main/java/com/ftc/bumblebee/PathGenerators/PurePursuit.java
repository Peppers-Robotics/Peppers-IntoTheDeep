...
public class PurePursuit {
    ...
    public Pose2d getNextHeadingPoint(boolean reversed){
        ...
        for(int i = 0; i < checkPoints.size() - 1; i++){
            ...
            Pose2d thisPos = new Pose2d(localizer.getCurrentPosition().x, localizer.getCurrentPosition().y, localizer.getCurrentPosition().h);
            ...
            if(checkPoints.elementAt(i).x == checkPoints.elementAt(i+1).x){
                ...
                delta = Radius * Radius - (x - localizer.getCurrentPosition().x) * (x - localizer.getCurrentPosition().x);
                if(delta < 0) continue;

                n1 = new Pose2d(x, Math.sqrt(delta) + localizer.getCurrentPosition().y);
                n2 = new Pose2d(x, - Math.sqrt(delta) + localizer.getCurrentPosition().y);

            } else if(checkPoints.elementAt(i).y == checkPoints.elementAt(i+1).y){
                double y = checkPoints.elementAt(i).y;

                delta = Radius * Radius - (y - localizer.getCurrentPosition().y) * (y - localizer.getCurrentPosition().y);
                if(delta < 0) continue;

                n1 = new Pose2d(Math.sqrt(delta) + localizer.getCurrentPosition().x, y);
                n2 = new Pose2d(-Math.sqrt(delta) + localizer.getCurrentPosition().x, y);
            ...
            if(Distance(n1, checkPoints.elementAt(i + 1)) < Distance(n2, checkPoints.elementAt(i + 1))){
                lastHeadingPose = new Pose2d(n1.x, n1.y, -getAngleFromTwoPoints(localizer.getCurrentPosition(), n1));
            } else {
                lastHeadingPose = new Pose2d(n2.x, n2.y, -getAngleFromTwoPoints(localizer.getCurrentPosition(), n2));
            }
